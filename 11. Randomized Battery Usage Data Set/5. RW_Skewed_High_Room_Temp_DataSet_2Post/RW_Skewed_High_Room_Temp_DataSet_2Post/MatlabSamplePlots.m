...
for i = 2:length(data.step)
    if (strcmp(data.step(i).comment,'pulsed load (discharge)')...
        ...
        if (strcmp(data.step(i).comment,'pulsed load (rest)') && ...
                (~strcmp(data.step(i-1).comment,'pulsed load (discharge)')))  
            counter1 = 1;
            counter2 = counter2+1;
...
inds = RWDis.repition{4}.indexes(1):RWDis.repition{14}.indexes(end);
