---
title: "Description of Left Skewed Random Walk Discharging Data Sets at 40C"
...
A set of four 18650 Li-ion batteries (Identified as RW21, RW22, RW23 and RW24) were continuously operated by repeatedly charging them to 4.2V and then discharging them to 3.2V using a randomized sequence of discharging currents between 0.5A and 5A. This type of discharging profile is referred to here as random walk (RW) discharging. A customized probability distribution is used in this experiment to select a new load setpoint every 1 minute during RW discharging operation. The custom probability distribution was designed to be skewed towards selecting lower currents. The ambient temperature at which the batteries are cycled was held at approximately 40C for these experiments.
...
## Files:
RW21.mat -> A Matlab data structure for Battery #RW21

RW21.Rda -> An R data frame containing the same data as the Matlab data structure

RW22.mat -> A Matlab data structure for Battery #RW22

RW22.Rda -> An R data frame containing the same data as the Matlab data structure

RW23.mat -> A Matlab data structure for Battery #RW23

RW23.Rda -> An R data frame containing the same data as the Matlab data structure

RW24.mat -> A Matlab data structure for Battery #RW16

RW24.Rda -> An R data frame containing the same data as the Matlab data structure
...
load("data/R/RW21.Rda")  # loading data
...
The differences in the voltage profiles observed in the previous plot are due to degradation of battery health over the experiment. As the battery ages, its charge storage capacity decreases, and it's internal resistance increases. 
