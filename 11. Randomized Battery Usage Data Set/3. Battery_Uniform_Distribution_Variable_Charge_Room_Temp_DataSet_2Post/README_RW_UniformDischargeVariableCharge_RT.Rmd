---
title: "Description of Room Temperature Random Walk Discharging Experiments with Variable Recharging Periods"
author: "Author: Brian Bole"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
---

### Points of Contact: chetan.s.kulkarni@nasa.gov

## Experiment Description

A set of four 18650 Li-ion batteries (Identified as RW1, RW2, RW7 and RW8) were continuously operated by repeatedly discharging them to 3.2V using a randomized sequence of discharging currents between 0.5A and 4A. This type of discharging profile is referred to here as random walk (RW) discharging. After each discharging cycle the batteries were charged for a randomly selected duration between 0.5 hours and 3 hours.  After every fifty RW cycles a series of reference charging and discharging cycles were performed in order to provide reference benchmarks for battery state health. 
...
## Files:
RW1.mat -> A Matlab data structure for Battery #RW1

RW1.Rda -> An R data frame containing the same data as the Matlab data structure

RW2.mat -> A Matlab data structure for Battery #RW2

RW2.Rda -> An R data frame containing the same data as the Matlab data structure

RW7.mat -> A Matlab data structure for Battery #RW7

RW7.Rda -> An R data frame containing the same data as the Matlab data structure

RW8.mat -> A Matlab data structure for Battery #RW8

RW8.Rda -> An R data frame containing the same data as the Matlab data structure

MatlabSamplePlots.m -> An example Matlab code that recreates all of the plots shown in this README document
...
### Batteries are cycled using five types of reference profiles:

1. A low current discharge at 0.04A is used to observe the batteries open circuit voltage (with a small discharging bias) as a function of SOC
...
2. A low current charge at 0.04A is used to observe the batteries open circuit voltage (with a small charging bias) as a function of SOC
    
    - This profile is identified in the **step** data structure with the **comment** field equal to **"low current charge"** 
    
    - Resting periods that occur immediately before and after the low current discharge are identified with the **comment** field = **"rest post low current discharge"** and **comment** field = **"rest post low current charge"** respectively  
    
3. A reference charge and discharge cycle is used to observe the battery capacity after every 50 RW cycles
...
4. A pulsed current discharge of fully charged batteries is performed after every 100 RW cycles in order to benchmark changes to battery transient dynamics. The pulsed current discharge consists of a 1A load applied for 10 minutes, followed by 20 minutes of no load. 
...
5. A pulsed current charge of recently discharged batteries is performed after every 100 RW cycles in order to benchmark changes to battery transient dynamics. The pulsed current charge consists of a 1A charging current applied for 10 minutes, followed by 20 minutes of rest. 
...
1. A battery is charged for a time interval that is randomly selected from the set {0.5 hours, 1 hours, 1.5 hours, 2 hours, 2.5 hours, or charge until full}. 

    - Batteries are charged at a 2A current until either the battery voltage reaches 4.2V or the charging time exceeds the allotted interval
    
    - If battery voltage reaches 4.2V then the system will switch to constant voltage charging. In this mode the charging current will be regulated to maintain 4.2V at the battery output until either the battery current drops below a lower threshold, or the charging time exceeds the allotted interval
    
    - RW charging steps are identified with the **comment** field = **"charge (random walk)"**
  
2. After charging, batteries are discharged to 3.2V using a randomized sequence of discharging loads between 0.5A and 4A. Discharging periods last 5 minutes each, with a <1s period of rest while a new discharging setpoint is selected.  
...
### Example Plots of Low Current Charge and Discharge Cycles

Sample plots for low current discharging and charging cycles are shown below.

```{r, echo=T, message=FALSE, cache=FALSE, fig.width = 7, fig.height = 8}

load("data/R/RW1.Rda") # loading data for battery RW1
...
# plot recorded current and voltage data from low current charge steps 

# find all of the steps with comment == 'low current charge at 0.04A'
LowCharSteps <- steps[steps$comment %in% 'low current charge',]

par(mfrow=c(2,1),mar=c(4,4,2,1)) # setup three rows of plots 
# plot voltage
plot(LowCharSteps$relativeTime[[1]]/3600,LowCharSteps$voltage[[1]], ylim = c(3,4.25),type="l",
     lwd=1.5,main = "Low current charge cycle",
     xlab = "Time (h)",ylab = "Voltage (V)")

# plot Current
plot(LowCharSteps$relativeTime[[1]]/3600,LowCharSteps$current[[1]], ylim = c(-.1,.1),type="l",
     lwd=1.5,xlab = "Time (h)",ylab = "Current (I)")
...
The differences in the voltage profiles observed in the previous plot are due to degradation of battery health over the experiment. As the battery ages, its charge storage capacity decreases, and its internal resistance increases. 
...
The pulsed load charging and discharging cycles that are run after every 100 random walk discharging cycles are plotted using.
...
## Looking at the Randomized Discharging and Recharging Data

The RW discharging profiles are composed of 5 minute periods of discharging the batteries at a particular current, with interleaved periods of resting the battery (current = 0). After the battery voltage falls below 3.2V the battery is rested for an hour (this step was added about 2 months into the experiment). Then the batteries are recharged for a randomly selected interval selected from the set {0.5 hours, 1 hours, 1.5 hours, 2 hours, 2.5 hours, or charge until full}.
...
# Identify comment codes of interest
CC <- c("discharge (random walk)", "rest (random walk)", 
...
The current and voltage plots above show the randomized nature of the charging duration. 

The duration of RW discharging periods is also seen to be much shorter for the last 10 RW discharging cycles than is seen in the first 10 cycles. This is due to the degradation of battery health over the experiment, which results in decreased charge storage capacity, and increased internal resistance. 
